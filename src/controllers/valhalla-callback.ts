...
export const valhallaCallbackController = async (
  ...
  try {
    const { channelId, guildId, success, archiveUrl } =
      req.body as ValhallaCallbackRequestBody;

    if (!channelId || !guildId) {
      return res
        .status(400)
        .json({ error: 'Missing channelId or guildId in request body' });
    }

    // Log the callback
    discordLogger(
      `Received valhalla-callback for channel ${channelId}. Success: ${success}`,
      client
    );
    ...
    if (!success) {
      return res
        .status(200)
        .json({ message: 'Received failed export notification' });
    }
    ...
    if (!botPermissions) {
      ...
      return res
        .status(500)
        .json({ error: 'Could not get permissions for bot in channel' });
    }
    ...
    discordLogger(
      `Valhalla callback detailed permission check: ${JSON.stringify(
        permissionDetails,
        null,
        2
      )}`,
    ...
    if (!botPermissions.has(PermissionFlagsBits.ManageChannels)) {
      ...
      return res.status(200).json({
        message:
          'Archive URL sent, but channel not moved due to permission issues',
        archiveUrl
      });
    }
    ...
    if (!targetCategory) {
      discordLogger(
        `Valhalla category not found: ${DISCORD_VALHALLA_CATEGORY_ID}`,
        client
      );
      ...
      return res.status(200).json({
        message:
          'Archive URL sent, but channel not moved because Valhalla category not found',
        archiveUrl
      });
    }
    ...
    discordLogger(
      `Error in valhalla-callback: ${JSON.stringify(errorDetails, null, 2)}`,
      client
    );
    return res
      .status(500)
      .json({ error: 'Internal server error', details: errorDetails });
  }
};
